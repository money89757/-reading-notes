...
#if 1
irqreturn_t key_handler(int irqnum,void *dev)//中断上半部
{
	global_irqnum = irqnum;
	mod_timer(&mytimer,jiffies + 4 * HZ / 100);
#if 0
...
void key_tasklet_handler(unsigned long data)//中断下半部的执行位置
{
	printk("my tasklet handler done\n");
}

void key_work_handler(struct work_struct *work)//中断下半部执行函数
{
	printk("my workqueue handler done\n");
}

void key_timer_handler(unsigned long data)
{
	if(global_irqnum == res_key1->start)
		key = 1;

	if(global_irqnum == res_key2->start)
		key = 2;
...
int key_probe(struct platform_device *key_dev)
{
	...
	init_timer(&mytimer);
	mytimer.function = key_timer_handler;
	add_timer(&mytimer);


	tasklet_init(&mytasklet,key_tasklet_handler,0);//初始化tasklet

	INIT_WORK(&mywork,key_work_handler);
